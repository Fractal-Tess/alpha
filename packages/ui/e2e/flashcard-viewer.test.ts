...
test.describe("FlashcardViewer", () => {
  ...
  test("displays flashcard with question on front", async ({ page }) => {
    ...
    await expect(card.locator("[data-testid='flashcard-back']")).toHaveClass(/opacity-0/);
  });
  ...
  test("flips card to show answer on click", async ({ page }) => {
    ...
    await expect(card.locator("[data-testid='flashcard-back']")).toHaveText("Paris");
  });
  ...
  test("shows empty state when no cards provided", async ({ page }) => {
    ...
    await expect(page.locator("[data-testid='empty-state']")).toContainText("flashcards");
  });
});
